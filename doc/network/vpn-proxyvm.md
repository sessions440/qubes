# VPN ProxyVM (`sys-vpn`) — Design & Configuration
...
Chain: `AppVM → sys-vpn → sys-firewall → sys-net`
...
qvm-service sys-vpn network-manager on
```

`netvm=sys-firewall` is required — `provides_network=True` only makes
`sys-vpn` an upstream for *other* qubes; it still needs its own upstream to
reach the internet. Omitting this leaves `sys-vpn` with no network at all.
Standard topology keeps `sys-firewall` between `sys-vpn` and `sys-net`
...
(`proton-ca.conf`, `proton-us.conf`). The filename becomes the WireGuard
interface name; longer names are silently truncated by the kernel.

## Step 5 — Import configs into `sys-vpn`

From the qube holding the downloaded configs:

```bash
qvm-copy-to-vm sys-vpn proton-ca.conf
```

In `sys-vpn`:

```bash
mv ~/QubesIncoming/<source-qube>/proton-*.conf ~/
sudo nmcli connection import type wireguard file ~/proton-ca.conf
sudo nmcli connection import type wireguard file ~/proton-us.conf

sudo nmcli connection modify proton-ca connection.autoconnect yes
sudo nmcli connection modify proton-us connection.autoconnect no
```
...
The `oifname "proton*"` wildcard is deliberate — it must match *every*
...
`oifname "proton*"` wildcard already used in the WireGuard ProxyVM's
...
```bash
qvm-prefs <appvm-name> netvm sys-vpn
```
...
# Confirm kill switch — bring the tunnel down in sys-vpn, then retry curl
# from the downstream AppVM. Expect a timeout, not a fallback to the real IP.
sudo nmcli connection down proton-ca
```

Restore:

```bash
sudo nmcli connection up proton-ca
```
...
```bash
sudo nmcli connection down proton-ca
sudo nmcli connection up proton-us
```
...
- **Community-maintained "sys-vpn template"** — does not exist as a
...
- WireGuard config filenames >15 chars get truncated as interface names —
  silently, no error.
- The nftables kill switch rule must use a wildcard (`proton*`) covering
...
- `provides_network=True` does not give `sys-vpn` its own uplink — `netvm`
